...
|---|---|---|
| iTunes Lookup API | HTTP GET `https://itunes.apple.com/lookup?id={id}&country=us` | App metadata, developer info, pricing — **4s timeout** |
| iTunes Search API | HTTP GET `https://itunes.apple.com/search?term=...&entity=software` | Keyword search, app discovery — **4s timeout** |
...
All calls to Apple's APIs in `app_import_service.py` use a **4-second hard timeout** (`_TIMEOUT = 4`).

| Function | Endpoint | Timeout | On timeout |
|---|---|---|---|
| `_search_itunes()` | `itunes.apple.com/search` | 4s | Returns `([], error_hint)` — never raises |
| `_get_full_app_details()` | `itunes.apple.com/lookup` | 4s | Returns `(None, error_hint)` — never raises |

Both functions return `(result, error_hint)` tuples:
- `error_hint` is `None` on success
- `error_hint` is a human-readable string on timeout or network failure

`search_apps()` surfaces `error_hint` only when there are **zero** local DB results (to avoid showing an error alongside valid local results).

`lookup_app()` maps `error_hint` into the `{"error": ...}` dict that the `/apps/import` route converts into a structured `error_hint` response field.

Exception handling hierarchy:
1. `TimeoutError` / `socket.timeout` → "Apple API timeout. Try again in a moment."
2. `urllib.error.URLError` wrapping a timeout reason → same
3. Any other exception → "Apple API unavailable. Showing local results only."

---

## Phase 0: Post-Import Hydration (On-Demand)

Triggered immediately after a new app is imported via `/apps/import` or `/apps/lookup/{id}`.

```
User imports app (URL / trackId / search)
  │
  ├── AppImportService.lookup_app()
  │     → writes App row to DB (is_new=True)
  │     → HTTP response returned to client immediately
  │
  └── threading.Thread(daemon=True).start()   ← non-blocking; HTTP response already sent
        │
        └── PostImportHydrationService.hydrate(app_id, app_store_id)
              │
              ├── Step 1: asyncio.run(ScraperWorker.scrape_app_full_details())
              │     → metadata refresh + iTunes RSS reviews + version history HTML
              │     → own event loop in new thread (safe: no running loop in daemon thread)
              │
              ├── Step 2: Keyword enrichment (each sub-step with own SessionLocal)
              │     ├── KeywordExtractionService.extract_for_app()
              │     ├── CompetitorKeywordService.mine_for_app()
              │     └── KeywordIntelligencePipeline.enrich_app()
              │
              └── Step 3: MetricSnapshotService.compute_for_app()
                    → download + revenue metric snapshot (own SessionLocal)
```

**Non-blocking guarantee:** `threading.Thread(daemon=True).start()` fires immediately after
the DB write, before the HTTP response is constructed. The request handler returns in <50ms.
Hydration completes in the background (typically 5–30s depending on Apple API latency).

**Failure isolation:** Each of the 3 steps in `hydrate()` is wrapped in its own `try/except`.
A failure in Step 1 (scrape) does not prevent Step 2 or Step 3 from running.

**Frontend:** The client redirects to `/apps/{id}?imported=1`, which shows a green dismissible
banner: "App imported successfully! Background enrichment is running..."

**Trigger:** Only for `is_new=True`. Re-importing an existing app does not re-trigger hydration.

**File:** `backend/app/services/post_import_hydration.py`
