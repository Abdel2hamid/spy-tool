...
Captures the full current schema as the Alembic baseline so future changes are
versioned instead of applied as ad-hoc DDL on every startup.

`upgrade()`:
  1. Creates every ORM-defined table via ``Base.metadata.create_all`` (idempotent).
  2. Re-applies the historical idempotent DDL that previously ran each boot in
     ``main._MIGRATIONS`` — the pg_trgm extension, GIN trigram indexes,
     performance indexes, legacy ADD COLUMN / CREATE TABLE IF NOT EXISTS
     statements, and the one-time ``apps.source`` backfill.

Every statement is IF NOT EXISTS / idempotent, so this is a no-op on databases
that already have the schema and fully builds a fresh database. It runs ONCE
(Alembic records it in alembic_version); it no longer runs on every startup.

CONVENTION: because this baseline uses create_all (which reflects the live
models), later revisions in this project should add schema with idempotent DDL
(``op.execute("... IF NOT EXISTS ...")``) so they stay safe on a fresh database
where create_all may already have built a newly-modelled table/column.
...
# Historical idempotent DDL, relocated verbatim from main._MIGRATIONS so the
# baseline reproduces the exact current schema (zero drift).
_BASELINE_DDL = [
...
def upgrade() -> None:
    # Run schema construction on an AUTOCOMMIT connection with per-statement
    # tolerance, mirroring the old main._run_migrations exactly. This makes the
    # one-time baseline safe on an already-provisioned production database: a
    # single pre-existing quirk can't abort the whole migration and block boot.
    # (Alembic still records the version stamp in its own transaction.)
    engine = op.get_bind().engine
    with engine.connect() as conn:
        conn = conn.execution_options(isolation_level="AUTOCOMMIT")
        # 1) Every ORM-defined table (idempotent — only creates what is absent).
        Base.metadata.create_all(conn)
        # 2) Non-ORM objects + historical idempotent DDL (pg_trgm extension, GIN
        #    indexes, performance indexes, one-time apps.source backfill).
        for sql in _BASELINE_DDL:
            try:
                conn.execute(text(sql))
            except Exception as exc:  # noqa: BLE001 — tolerant one-time cutover
                log.warning("baseline DDL skipped (%s): %s", type(exc).__name__, exc)


def downgrade() -> None:
    # Baseline: dropping the entire schema is intentionally unsupported.
